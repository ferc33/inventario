...
class Product(models.Model):
    ...
    codigo_proveedor = models.CharField(max_length=50, verbose_name='Codigo proveedor')
    def __str__(self):
        return f'{self.name} ({self.category.name})'
#Datos del datatables de producto /product/
    def toJSON(self):
        item = model_to_dict(self)        
        item['full_name'] = self.__str__()
        item['codigo_proveedor'] = self.codigo_proveedor
        item['category'] = self.category.toJSON()
        item['image'] = self.get_image()
        item['pvp'] = f'{self.pvp:.2f}'
        # Añadir el campo 'codigo_proveedor'
...
class Project(models.Model):
    ...
    def toJSON(self):
        item = model_to_dict(self)
        item['architect'] = self.architect.toJSON()
        item['images'] = [x.toJSON() for x in self.images.all()]
        return item
...
class Remito(models.Model):
    ...
    def toJSON(self):
        ...
        item['total'] = f'{self.total:.2f}'
        item['date_joined'] = self.date_joined.strftime('%Y-%m-%d')
        item['remitoproduct'] = [i.toJSON()
                                 for i in self.remitoproduct_set.all()]
        return item

    def delete(self, using=None, keep_parents=False):
        for detail in self.remitoproduct_set.all():
            detail.product.stock += detail.cant
            detail.product.save()
        super(Remito, self).delete()

    def calculate_invoice(self):
        subtotal = self.remitoproduct_set.all().aggregate(result=Coalesce(
            Sum(F('price') * F('cant')), 0.00, output_field=FloatField())).get('result')
        self.subtotal = subtotal
        self.total_iva = self.subtotal * float(self.iva)
        self.total = float(self.subtotal) + float(self.total_iva)
...
class RemitoProduct(models.Model):
